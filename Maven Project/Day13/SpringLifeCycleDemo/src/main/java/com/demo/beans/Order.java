...
public class Order implements BeanNameAware,BeanFactoryAware, InitializingBean,DisposableBean {
	...
	public void afterPropertiesSet() {
		...
		List<Double>amtlist=plist.stream().map(product->product.getQty()*product.getPrice()).collect(Collectors.toList());
		
		//calculate total amount
		this.orderamount=amtlist.stream().reduce(0.0, (acc,amt)-> acc+amt);
		System.out.println("In afterPropertiesSet of Innitializing beans ");
	}
	public void myInit() {
