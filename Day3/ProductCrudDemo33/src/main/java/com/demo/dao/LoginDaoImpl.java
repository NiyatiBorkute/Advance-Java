...
public class LoginDaoImpl implements LoginDao{
     ...
     static {
    	 ...
    	 try {
			...
			seluser=conn.prepareStatement("select id, uname,email,role from user where uname=? and password=?");
	...
	public MyUser checkUser(String uname, String pass) {
		try {
			...
			if(rs.next()) {
				MyUser user=new MyUser((1), rs.getString(2),rs.getString(3),rs.getString(4));
				return user;
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
